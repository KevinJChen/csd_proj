...
/*
 int nnls_(doublereal *a, integer *mda, integer *m, integer *n, 
 doublereal *b, doublereal *x, doublereal *rnorm, doublereal *w,
...
void nnls_compute(NNLS_ARENA *nnls, double *A, double *b, int copy_a)
{
    ...
    int A_size = sizeof(double) * nnls->m * nnls->n;
    double *nnls_A;
    
    if (copy_a != 0) {
        nnls_A = nnls->A;
        memcpy(nnls_A, A, A_size);
    } else {
        nnls_A = A;
    }
    ...
    nnls_c(nnls_A, &(nnls->m), &(nnls->m), &(nnls->n), b, nnls->x,
           &(nnls->rnorm), nnls->w, nnls->zz, nnls->index, &(nnls->mode));
...
NNLS_ARENA *nnls_initialize(double *A, int m, int n)
{
    ...
    NNLS_ARENA *nnls = NULL;
    
    double *a_scratch = malloc(sizeof(double) * m * n);
    if (a_scratch == NULL) {
        fprintf(stderr, "Unable to allocate memory for NNLS scratchwork.\n");
        return NULL;
    }
    
    nnls = malloc(sizeof(NNLS_ARENA));
    if (nnls == NULL) {
        fprintf(stderr, "Unable to allocate memory for NNLS computation.\n");
        return NULL;
    }
    ...
    nnls->m = m;
    nnls->n = n;
    nnls->w = malloc(sizeof(double)*(n+1));
    nnls->x = malloc(sizeof(double)*(n+1));
    nnls->mode = 0;
    nnls->rnorm = 0;
    nnls->index = malloc(sizeof(int) * (n+2));
    nnls->zz = malloc(sizeof(double) * (m+1));
    
    if (nnls->x == NULL || nnls->w == NULL || 
        nnls->index == NULL || nnls->zz == NULL) {
        fprintf(stderr, "Unable to allocate memory for NNLS computation.\n");
        return NULL;
    }
